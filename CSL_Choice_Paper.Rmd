...
A 2x2 ANOVA was performed in order to evaluate the effect of decision (repeat vs switch) and group (colour vs shape) on the time taken to make a task choice. Results show no significant main effects of Group (`r choiceRT.res$full_result$Group`) and decision (`r choiceRT.res$full_result$repswitch`), as well as no significant interaction between group and decision (`r choiceRT.res$full_result$Group_repswitch`).
...
behavData %>%
  ...
  geom_line(aes(x=repswitch, y = mean, colour = Group, group = Subject), alpha = 0.5) +
  geom_point(data = choiceRTAvg, size =3.5) +
  facet_grid(.~Group) +
  scale_color_manual(values=c("green4", "gray42")) +
  lims(y = c(0,500)) +
  theme(panel.grid.major = element_blank(), 
...
behavData %>%
  ...
  geom_boxplot(aes(x = GroupNum, y = mean, fill = Group),outlier.shape = NA, alpha = .5, width = .1, colour = "black") +
  scale_color_manual(values=c("green4", "gray42")) +
  scale_fill_manual(values=c("green4", "gray42")) +
  labs(y = "Switch Rate (%)") +
  lims(y=c(0,100)) +
