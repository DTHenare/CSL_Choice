...
allData %>%
  ...
  ggplot(., aes(sample,diff)) +
    ...
    geom_text(x=NtStart+winSize/2, y=2, label="NT") +
    geom_text(x=PdStart+winSize/2, y=-1.8, label="PD") +
    scale_colour_manual(values=c("green4", "gray42"))+
    scale_x_continuous(name ="Latency (ms)", expand = c(0, 0)) +
    scale_y_reverse(name =expression(paste("Amplitude (",mu,"v)")), expand = c(0, 0)) +
...
allData %>%
  ...
  ggplot(., aes(sample,diff)) +
    ...
    geom_line(aes(colour = Group),size=1) +
    ggtitle("Search task") +
    geom_text(x=NtStart+winSize/2, y=3, label="NT") +
    geom_text(data = . %>% filter(Stimulus  == "Distractor"),x=PdStart+winSize/2, y=-2, label="PD") +
    scale_colour_manual(values=c("green4", "gray42"))+
