...
include_graphics("Figures/CSL-Choice trial displays.pdf", auto_pdf = TRUE)
```
...
include_graphics("Figures/CSLChoice Trial Proc and interblock feedback.pdf", auto_pdf = TRUE)
```
...
laterality.NTO  <- allData %>%
  ...
  mutate( text = paste("$\\Delta M = ", as.character(round(mV,2)), "muV, SE = ", as.character(round(se,2)), "$", sep=""))
...
intr.tdnt.se <- searchNtTD %>%
  ...
  mutate( text = paste("$\\Delta M = ", as.character(round(mean,2)), "muV, SE = ", as.character(round(se,2)), "$", sep=""))
...
grid.arrange(choicePercent,choiceRTPlot, ncol = 2, nrow = 1)
```
