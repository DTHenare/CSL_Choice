...
allData$Object = factor(allData$Object, labels = c("ColourDistractor", "ColourNone", "Lateralised Shape", "Lateralised Colour", "ColourTarget", "ShapeDistractor", "ShapeNone", "ShapeTarget"))
```
...
learnNtShape <- allData %>%
  ...
  filter(Event == "Learn" & Reject == 0 & sample>NtStart & sample < NtEnd & Object == "Lateralised Shape") %>%
...
learnNtCol <- allData %>%
  mutate(sample = sample-baseline) %>%
  filter(Event == "Learn" & Reject == 0 & sample>NtStart & sample < NtEnd & Object == "Lateralised Colour") %>%
  group_by(Object,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
...
learnPdShape <- allData %>%
  mutate(sample = sample-baseline) %>%
  filter(Event == "Learn" & Reject == 0 & sample>PdStart & sample < PdEnd & Object == "Lateralised Shape") %>%
  group_by(Object,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
...
learnPdCol <- allData %>%
  mutate(sample = sample-baseline) %>%
  filter(Event == "Learn" & Reject == 0 & sample>PdStart & sample < PdEnd & Object == "Lateralised Colour") %>%
  group_by(Object,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
