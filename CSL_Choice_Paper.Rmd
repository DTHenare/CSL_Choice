...
(ref:searchERPCap) Subtracted ERPs showing the lateralised response for each group across the three different search display conditions. The top panel shows equivalent NT components (`r as.character(round(NtStart))`ms to `r as.character(round(NtEnd))`ms) elicited by both groups when a target is presented without distraction. The middle panel shows the attenuated NT for participants in the colour group when a target is presented alongside a colour distractor. The bottom panel shows the PD component (`r as.character(round(PdStart))`ms to `r as.character(round(PdEnd))`ms) elicited by lateralised colour distractors for participants in the colour group but not the shape group.
...
We also observe substantial persistence of previously reported ERP effects in our version of the task. The first component of interest was the NT, a lateralised negative deflection which is elicited by target stimuli in a search display. In our results we observe that when participants perform visual search for a shape target while colour distractors are absent, then both groups of participants produce an equivalent NT to the target. This is consistent with the suggestion that in the absence of a coloured distractor, both groups were equally proficient at directing their attention to the shape singleton. When focusing on amplitude differences in the NT component, it is typically the case that increased NT amplitude reflects enhanced target selection processes. NT amplitude is greater for example in response to threatening stimuli [@weymar_specific_2013], highly rewarding stimuli [@kiss_reward_2009], and stimuli whose processing has been facilitated by intertrial priming [@tollner2008electrophysiological] (although see @mazza2009attention, @mazza2011temporal, @mazza2013multiple, and @naughtin2016early for examples in which the relationship is more complex). In both our task we see that if participants perform shape search in the presence of a coloured distractor, the NT amplitude is significantly reduced for participants who have a history of colour categorisation. This is therefore consistent with the idea that a history of colour categorisation leads to incraesed attention capture by colour singletons and therefore an impaired ability to efficiently direct attention to the shape target.
