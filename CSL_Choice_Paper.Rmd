...
Participants were also informed to attempt to keep the task selection balanced at around 50% search and 50% choice trials throughout the experiment. In order to measure this we calculated task choice across the task and show that participants typically succeeded in maintaining approximately 50% of each trial across the task. This is shown in figure\ \@ref(fig:choseSearch).
...
scale_y_continuous(limits = c(0,1100), expand = c(0, 0)) +
scale_x_discrete(labels = c("Rep", "Switch")) +
labs(y = "Response time (ms)", x = "Learn Task") +
theme(panel.grid.major = element_blank(),
panel.grid.minor = element_blank(),
panel.background = element_blank(),
axis.line = element_line(colour = "black"),
...
In our results, we see that participants in the colour group had a repetition bias indicating that they were unable to randomly select between trial types and instead were more likely to repeat the previous trial type than switch. This was different to participants in the shape group who showed no evidence of a bias in their choices. This implies that participants who are performing shape categorisation in the learn task may be incurring smaller costs as a result of task switching, and therefore do not attempt to avoid task switches to the same degree as participants in the colour group. This suggestion is supported by the fact that response time costs associated with a task switch are only present in the colour group when they switch from a learn trial to a search trial. We see no such switch costs for those in the shape group and no significant cost for the colour group when switching from the search to learn task. The relative lack of switch costs in our task confirms that the increased top-down control afforded by the voluntary task selection procedure is allowing participants to mitigate the costs that are typically usually observed during task switching. In spite of this maximal top-down control however, participants in the colour group are still incurring some task switch costs and show a repetition bias as a result.
