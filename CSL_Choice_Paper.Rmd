...
allData$Stimulus <- recode(allData$Stimulus, "Target (None mid)" = "Target alone", "Target (Distractor mid)"="Target (Dist mid)", "Distractor (Target mid)" = "Distractor (Targ mid)")
...
allData$Stimulus = factor(allData$Stimulus, levels = c("Target alone", "Target (Dist mid)", "Distractor (Targ mid)"))
...
In the categorisation task, our ERP analyses focused on the lateralised response to each object (either the shape singleton or the colour singleton). For each ERP there were two time windows of interest, the Nt time window (`r sprintf("%i to %ims", NtStart, NtEnd)`) and the Pd time window (`r sprintf("%i to %ims", PdStart, PdEnd)`). 
...
whereas the lateralised respone to the colour singleton is more negatvie for those in colour categoriser group that the shape categoriser group (`r learn.NtCol.results$full$Group_Contra`). There is no interaction between electrode and group in the Pd time range for either the lateralised shape ERP (`r learn.PdShape.results$full$Group_Contra`) or lateralised colour ERP (`r learn.PdCol.results$full$Group_Contra`).
...
allData %>%
  ...
  ggplot(., aes(sample,diff)) +
    geom_rect(aes(xmin = NtStart, xmax = NtEnd, ymin = -Inf, ymax = 0), fill = "lightblue1") +
    geom_rect(aes(xmin = PdStart, xmax = PdEnd, ymin = 0, ymax = Inf), fill = "mistyrose") +
...
(ref:learnERPsbyRepxSw) Subtractracted ERPs showing the lateralized response to the shape singleton (top) and color singleton (bottom) for each group in the categorization task. Responses for trials which were a repeated search trial are plotted separately from those that were performed after a task switch.

```{r learnERPsbyRepxSw, fig.cap="(ref:learnERPsbyRepxSw)"}
#Subtracted
allData %>%
  filter(Event == "Learn", Reject == 0) %>%
  mutate(sample = sample*2-baseline) %>%
  group_by(Object,sample,Contra,Group, TrialType) %>%
  summarise(mean = mean(voltage)) %>%
  spread(Contra, mean) %>% 
  mutate(diff = Contralateral - Ipsilateral) %>%
  ggplot(., aes(sample,diff)) +
...
ggsave("LearnSubtracted_byRep.pdf", dpi = 300, width = 30, height = 20, units = c("cm"))
```

(ref:learnERPsbyfirstxsec) Subtractracted ERPs showing the lateralized response to the shape singleton (top) and color singleton (bottom) for each group in the categorization task. Responses for trials which were performed in the first half of the experiment are plotted separately from those that were performed in the second half.

```{r learnERPsbyfirstxsec, fig.cap="(ref:learnERPsbyfirstxsec)"}
#Subtracted
allData %>%
  filter(Event == "Learn", Reject == 0) %>%
  mutate(sample = sample*2-baseline) %>%
  group_by(Object,sample,Contra,Group, Timepoint) %>%
  summarise(mean = mean(voltage)) %>%
  spread(Contra, mean) %>% 
  mutate(diff = Contralateral - Ipsilateral) %>%
  ggplot(., aes(sample,diff)) +
...
ggsave("LearnSubtracted_byTime.pdf", dpi = 300, width = 30, height = 20, units = c("cm"))
```
...
searchNtTO <- allData %>%
  mutate(sample = sample*2-baseline) %>%
  filter(Event == "Search" & LatStim != "None" & Reject == 0 & sample>NtStart & sample < NtEnd & Stimulus == "Target (None mid)") %>%
...
searchNtTD <- allData %>%
  mutate(sample = sample*2-baseline) %>%
  filter(Event == "Search" & LatStim != "None" & Reject == 0 & sample>NtStart & sample < NtEnd & Stimulus == "Target (Distractor mid)") %>%
  group_by(Stimulus,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
...
searchNtDT <- allData %>%
  mutate(sample = sample*2-baseline) %>%
  filter(Event == "Search" & LatStim != "None" & Reject == 0 & sample>NtStart & sample < NtEnd & Stimulus == "Distractor (Target mid)") %>%
  group_by(Stimulus,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
...
searchPdTO <- allData %>%
  mutate(sample = sample*2-baseline) %>%
  filter( Event == "Search" & LatStim != "None" & Reject == 0 & sample>PdStart & sample < PdEnd & Stimulus == "Target (None mid)") %>%
  group_by(Stimulus,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
...
searchPdTD <- allData %>%
  mutate(sample = sample*2-baseline) %>%
  filter(Event == "Search" & LatStim != "None" & Reject == 0 & sample>PdStart & sample < PdEnd & Stimulus == "Target (Distractor mid)") %>%
  group_by(Stimulus,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
...
searchPdDT <- allData %>%
  mutate(sample = sample*2-baseline) %>%
  filter( Event == "Search" & LatStim != "None" & Reject == 0 & sample>PdStart & sample < PdEnd & Stimulus == "Distractor (Target mid)") %>%
  group_by(Stimulus,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
...
Results for the lateralised distractor show that while there is no effect of group on the Nt component (`r search.NtDT.results$full$Group_Contra`), the Pd is significantly larger in the colour-trained group than the shape trained group (`r search.PdDT.results$full$Group_Contra`). 

#### Target response - Distractor present trials

Results for the lateralised target in the presence of a distractor show that the Nt is significantly larger for participants in the shape trained group than the colour trained group (`r search.NtTD.results$full$Group_Contra`), whereas there is no effect of group on Pd amplitude (`r search.PdTD.results$full$Group_Contra`). 

#### Target response - Distractor absent trials

In the target only displays there is no effect of group assignment on either the Nt (`r search.NtTO.results$full$Group_Contra`) or the Pd component (`r search.PdTO.results$full$Group_Contra`).
...
allData %>%
  ...
  ggplot(., aes(sample,diff)) +
    ...
    geom_rect(data = . %>% filter(Stimulus  == "Distractor (Targ mid)"),aes(xmin = PdStart, xmax = PdEnd, ymin = 0, ymax = Inf), fill = "lemonchiffon") +
    geom_line(aes(colour = Group),size=1) +
    ...
    geom_hline(yintercept = 0,linetype = "dashed") +
    theme_apa() +
    theme(panel.spacing.y = unit(3, "lines"), text= element_text(size=28),
          axis.text.x = element_text(size = 26),
          axis.text.y = element_text(size = 26),
          legend.title = element_text( size = 26),
...
ggsave("SearchSubtracted.pdf", dpi = 300, width = 30, height = 30, units = c("cm"))
```

(ref:searchERPbyRepxSw) Subtractracted ERPs showing the lateralized response to the colored distractor (top), the shape target (middle), and the shape target in the absence of distraction (bottom) for each group in the search task. Responses for trials which were a repeated search trial are plotted separately from those that were performed after a task switch.

```{r searchERPbyRepxSw, fig.cap="(ref:searchERPCap)"}
allData %>%
  filter( Event == "Search" & LatStim != "None" & Reject == 0) %>%
  mutate(sample = sample*2-baseline) %>%
  group_by(Stimulus,sample,Contra,Group,TrialType) %>%
  summarise(mean = mean(voltage)) %>%
  spread(Contra, mean) %>% 
  mutate(diff = Contralateral - Ipsilateral) %>%
  ggplot(., aes(sample,diff)) +
...
ggsave("SearchSubtracted_byRep.pdf", dpi = 300, width = 30, height = 20, units = c("cm"))
```

(ref:searchERPbyfirstxsec) Subtractracted ERPs showing the lateralized response to the colored distractor (top), the shape target (middle), and the shape target in the absence of distraction (bottom) for each group in the search task. Responses for trials which were performed in the first half of the experiment are plotted separately from those that were performed in the second half.

```{r searchERPbyfirstxsec, fig.cap="(ref:searchERPbyfirstxsec)"}
allData %>%
  filter( Event == "Search" & LatStim != "None" & Reject == 0) %>%
  mutate(sample = sample*2-baseline) %>%
  group_by(Stimulus,sample,Contra,Group,Timepoint) %>%
  summarise(mean = mean(voltage)) %>%
  spread(Contra, mean) %>% 
  mutate(diff = Contralateral - Ipsilateral) %>%
  ggplot(., aes(sample,diff)) +
...
ggsave("SearchSubtracted_byTime.pdf", dpi = 300, width = 30, height = 20, units = c("cm"))
```

### Permutation testing

```{r permutationTesting, echo=FALSE}
nPermutes = 100

if (FALSE) {
for (permutation in 1:2) {
#Shuffle the $X condition
permEpochInfo = epochInfo
permEpochInfo$Hemisphere = sample(permEpochInfo$Hemisphere, replace=FALSE)

#recreate allData in permAllData
scalpData = rbind(lHemData,rHemData)
origEpochInfo = rbind(permEpochInfo,permEpochInfo)

permAllData <- cbind(origEpochInfo, scalpData)
permAllData <- gather(permAllData, "sample", "voltage", gathercols, factor_key = TRUE)

#Tidy variable names etc. and create any necessary variables - could use unite
permAllData$sample <- as.integer(substring(permAllData$sample,2))
permAllData <- permAllData %>% mutate(Contra = ifelse(Hemifield==Hem, "Ipsilateral", "Contralateral"))
permAllData <- permAllData %>% mutate(Stimulus = paste(LatStim," (",MidStim," mid)"))
permAllData <- permAllData %>% mutate(Object = as.factor(paste(Group,LatStim, sep="")))
permAllData$Object <- recode(permAllData$Object, ShapePredictor = "Shape", ColourPredictor="Colour", ShapeNonPred = "Colour", ColourNonPred = "Shape")

searchNtTD <- permAllData %>%
  mutate(sample = sample*2-baseline) %>%
  filter(Event == "Search" & LatStim != "None" & Reject == 0 & sample>NtStart & sample < NtEnd & Stimulus == "Target (Distractor mid)") %>%
  group_by(Stimulus,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
