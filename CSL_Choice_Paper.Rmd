...
A 2x2 ANOVA was also carried out in order to evaluate whether there were effects of group and task on participant accuracy. The results did not show any significant difference in accuracy between the shape group ($M$ = `r as.character(round(group.acc$mean[2],2))`%, $SE$ = `r as.character(round(group.acc$se[2],2))`) and the shape group (`r paste("$M$ = ", as.character(round(group.acc$mean[1],2)), "%, $SE$ = ", as.character(round(group.acc$se[1],2)), ", " ,acc.res$full_result$Group, sep = "")`), nor any significant difference in accuracy on the categorisation task ($M$ = `r as.character(round(task.acc$mean[1],2))`%, $SE$ = `r as.character(round(task.acc$se[1],2))`) and the search task (`r paste("$M$ = ", as.character(round(task.acc$mean[2],2)), "%, $SE$ = ", as.character(round(task.acc$se[2],2)), ", " ,acc.res$full_result$Procedure_Trial_, sep = "")`). There was also no significant interaction between group and task on accuracy (`r acc.res$full_result$Group_Procedure_Trial_`).
...
Across both trial types, the colour group's response times (`r group.srt$text[1]`) did not differ significantly from participants in the shape group (`r paste(group.srt$text[2], ", ", searchRT.res$full_result$Group,sep="")`), however participants in both groups did appear to respond significantly faster in the distractor absent trials (`r task.srt$text[2]`) than in the distractor present trials (`r paste(task.srt$text[1], ", ", searchRT.res$full_result$TrialType,sep="")`). An interaction between trial type and group demonstrates that distractor presence had a significantly larger effect on participants in the colour group (`r intr.srt.text[1]`) than those in the shape group (`r paste(intr.srt.text[2], ", ", searchRT.res$full_result$Group_TrialType, sep ="")`). This effect of group assignment on distractor cost is shown in figure\ \@ref(fig:DistIntViolin).

When it comes to search task accuracy we did not observe any differences between the shape group (`r group.sacc$text[2]`) and the colour group (`r paste(group.sacc$text[1], ", ", gsub("= .000\\$", "< .001\\$",gsub("= 0.00\\$","< 0.01\\$", searchAcc.res$full_result$Group)),sep="")`), nor and differences between the target only trials (`r task.sacc$text[2]`) and target and distractor trials (`r paste(task.sacc$text[1], ", ", gsub("= .000\\$", "< .001\\$",gsub("= 0.00\\$","< 0.01\\$",searchAcc.res$full_result$TrialType)),sep="")`). There was also no interaction between group and trial type on accuracy (`r searchAcc.res$full_result$Group_TrialType`).
