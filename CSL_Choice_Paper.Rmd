...
NtStart = grandTarget$sample[grandTarget$diff == min(grandTarget$diff)]-50
NtEnd = grandTarget$sample[grandTarget$diff == min(grandTarget$diff)]+50
grandDist <- allData %>%
  ...
  spread(Contra, mean) %>% 
  mutate(diff = Contralateral - Ipsilateral)
PdStart = grandDist$sample[grandDist$diff == max(grandDist$diff)]-50
PdEnd = grandDist$sample[grandDist$diff == max(grandDist$diff)]+50
```
...
searchNtTO <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(Event == "Search" & LatStim != "None" & searchRej == 0 & sample>NtStart & sample < NtEnd & Stimulus == "Target only") %>%
...
searchNtTD <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(Event == "Search" & LatStim != "None" & searchRej == 0 & sample>NtStart & sample < NtEnd & Stimulus == "Target") %>%
  group_by(Stimulus,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
...
searchNtDT <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(Event == "Search" & LatStim != "None" & searchRej == 0 & sample>NtStart & sample < NtEnd & Stimulus == "Distractor") %>%
  group_by(Stimulus,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
...
searchPdDT <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter( Event == "Search" & LatStim != "None" & searchRej == 0 & sample>PdStart & sample < PdEnd & Stimulus == "Distractor") %>%
  group_by(Stimulus,Subject,Contra,Group) %>%
  summarise(mV = mean(voltage))
