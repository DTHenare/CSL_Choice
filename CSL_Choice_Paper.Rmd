...
abstract: |
  A typical visual scene contains vast amounts of information, and only a fraction of this can be fully processed by the human visual system. To understand how our attention is directed to one object over another, research has historically focussed on the interaction between bottom-up stimulus features, and an individual's top-down goals. In this study we evaluated how a third factor, an individual's selection history, interacts with top-down control mechanisms during visual search. Participants in our task were assigned to one of two groups in which they develop a history of shape selection or colour selection in one task, while performing visual search for a shape singleton in a second task. A voluntary task selection procedure allowed participants the choice of which task they would perform on any given trial thereby maximising their opportunity for top-down preparation. We recorded EEG while participants performed these tasks and extracted lateralised ERP components as indices of target selection (NT) and distractor suppression (PD). Our results showed that selection history continued to guide attention during visual search, even when top-down control mechanisms were maximised with a voluntary task selection procedure. For participants with a history of colour selection, the NT component elicited by a shape target is attenuated if it is accompanied by a colour distractor, and the distractor itself elicits a larger PD component. Additionally, task switching results reveal that participants in the colour group have larger, asymmetric switch costs which implies increased competition between the two task sets. Our results support the notion that selection history is a significant factor in attention guidance, orienting the visual system reflexively to objects that contradict an individual's current goals - even when these goals are intrinsically selected and prepared ahead of time.
...
choicePercent <- behavData %>%
  ...
  labs(y = "Chose search (%)") +
  lims(y=c(0,100)) +
  annotate_custom(grob = textGrob(label = "A"), xmin = -5, xmax = 0, ymin = 110, ymax=135) +
...
final <- grid.arrange(choicePercent,choiceRTPlot, ncol = 2, nrow = 2,  widths = c(2,1), layout_matrix = rbind(c(1,1),c(2,2)))
ggsave("Figures/taskChoice.jpg", plot = final, dpi = 600, width = 8, height = 12, units = "cm")
```
...
Notably in our case, participants in the shape group were not displaying a similar switch cost asymmetry. This suggests that the between-task competition present in previous task switching experiments, and present in the colour group of our task, was reduced for participants in the shape group. Importantly this is not because of a change in task difficulty between the groups - the shape group's response times show that they also find the search task more difficult than the categorisation task. Instead, while task difficulty remains the same for both groups, the switch cost asymmetry is absent for the shape group whose target is a shape singleton in both tasks. This may suggest that between-task competition is modulated not only by response compatibility (as described by @yeung2010bottom), but by modulating the congruency between the attentional templates used in each task. A thorough understanding of the factors that modulate between-task competition will require further investigation, however, our results suggest that such work may find utility in expanding on the paradigm used here to control for task difficulty while manipulting between-task competition.
