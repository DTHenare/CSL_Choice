...
Evidence that voluntary task switching procedures enable greater top-dwon control compared to cued task swithing procedures comes from both behavioural and electrophysiological results. 
...
For the analysis of preparatory activity, we followed the EEG processing procedures used by previous investigations of cued and voluntary task switching. Data were rereferenced to the average of the mastoids and segmented into 1200ms epochs around the onset of the fixation cross in each trial. The epochs start with a 200ms baseline period prior to the onset of the fixation, and extend 1000ms post fixation corresponding to the entire preparatory period and extending 500ms into the poststimulus period. Data were then filtered using a lowpass value of 35Hz, and epochs were rejected if they contained activity exceeding +/-200mv in any channel, +/-75mv in EOG channels, or improbable data (based on EEGLAB's pop_jointprob function with a 6SD threshold). Trials where participants repsonded incorrectly were also excluded. A fronto-central cluster of electrodes and a posterior cluster of electrodes weres then created based on the channels used by @poljac_dissociable_2014 and we extracted component amplitudes by averaging the 300ms period directly preceding stimulus onset.
...
An independent samples t test was used to compare the CNV amplitude between our data and the data from @kadel2017selection, and a second independent samples t test was was to compare the posterior positivity bewteen the two experiments.
...
Comparison of the preparatory activity in our experiment to the preparatory activity recorded by @kadel2017selection is shown in figure, we see a significantly larger CNV at frono-central electrodes in our voluntary task switching procedure (`r front.stats$full_result`). We also see a significantly larger positivity at posterior electrods in the cued task switching procedure used by @kadel2017selection (`r post.stats$full_result`).
