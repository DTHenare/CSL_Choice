...
ggsave("taskChoice.jpg", plot = final, dpi = 600, width = 8, height = 12, units = "cm")
```
...
An interesting feature of the switch costs described above is the asymmetry observed in the colour group. Switch costs were significantly larger when the colour group switched from the search task to the categorisation task, than in the reverse situation where they switched from categorisation to search. This kind of switch cost asymmetry has been described in previous task switching paradigms where the two tasks differ in difficulty [@arrington2014voluntary]. Somewhat paradoxically it has been shown that switch costs tend to be larger for the easier task, and this is true for general task switching paradigms [@alport199417] as well as in various types of voluntary task selection [@yeung2010bottom;@millington2013between]. While participant accuracy in our two tasks did not differ significantly, we did see a significant slowing of responses to the search task suggesting that it may have been the more difficult of the two tasks. Larger switch costs for the categorisation task are therefore consistent with the finding that switch costs are usually larger for easier tasks. 

The main explanation for participants' preference for the difficult task and reduced costs when switching to the difficult task is between-task competition. In a task switching procedure, participants have two task sets competing for representation while they perform any given trial. By default this competition is weighted in favour of the easier task, and performance of the difficult task requires active enhancement of its weaker task set. As a consequence of enhancing the strength of the difficult task set, participants in a voluntary selection procedure are more likely to repeat the difficult task in a subsequent trial, and will take reduced switch costs when they select the difficult task. @yeung2010bottom provided evidence for this explanation with a voluntary task selection procedure where response compatibility was manipulated in order to modulate between-task interference. Their results 

When participants are switching between two different tasks, the task sets are competing for representation and interfering with one another. Difficult tasks are presumed to be weaker in this competition by default and, therefore, receive greater enhancement during a trial in order to overcome interference from the easy task. As a result of enhancing the difficult task set, participants become more likely to repeat the task in the subsequent trial and will take smaller switch costs.

interfere with one another and, therefore, in a voluntary switching procedure a participants choice is the outcome of competition between these task sets. 
By default, it is presumed that the more difficult task is the weaker of the two tasks, but that its task set is therefore enhanced by participants 
@millington2013between suggested that this effect is caused by between-task competition in which the task set of a difficult task needs to be imposed more strongly, making it harder to suppress when switching back to the easier task.
